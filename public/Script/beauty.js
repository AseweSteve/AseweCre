...
let parent = document.getElementById("rkbox");
...
function bea(a) {
    ...
    a.forEach(function(p) {
        if (p.department == "beauty & grooming") {
    ...
    if (products_cart.length) {
        let no = document.getElementsByClassName("numb");
        no[0].textContent = products_cart.length;
...
function search() {
    let brand = document.getElementById("bd").value;
    if (brand) {
        let arr = cliq.filter(function(el) {
            return el.brand.toLowerCase() == brand.toLowerCase();
        });
        bea(arr);
    }
}